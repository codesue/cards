"""Placeholder Model Card Toolkit library for local developmoent since I can't
install MLMD on my M1 yet. See https://github.com/google/ml-metadata/issues/143.

Code in this file is based on Model Card Toolkit, which is copyright 2020 Google LLC
...
def _validate_json_schema(
  ...
  logging.warning("Skipping schema validation since Model Card Toolkit isn't installed.")
...
class ModelCard(BaseModelCardField):
  ...
  def to_json(self) -> str:
    ...
    model_card_dict[
        _SCHEMA_VERSION_STRING] = _get_latest_schema_version()
  ...
  def from_json(self, json_dict: Dict[str, Any]) -> None:
    ...
    _validate_json_schema(json_dict)
    self.clear()
